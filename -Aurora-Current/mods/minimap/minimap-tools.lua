...
function minimap:SetupMinimap(cluster)
    ...
    AU.common.KillFrame(MinimapCluster)
    AU.common.KillFrame(MinimapBackdrop)
    AU.common.KillFrame(MinimapZoomIn)
    AU.common.KillFrame(MinimapZoomOut)
...
function minimap:CreateTopPanel(cluster)
    ...
    local timeText = AU.ui.Font(panel, 11, '00:00', {1, 1, 1}, 'RIGHT')
    timeText:SetPoint('RIGHT', panel, 'RIGHT', -11, 0)

    local zoneText = AU.ui.Font(panel, 11, 'Zone', {1, 0.82, 0}, 'LEFT')
    zoneText:SetPoint('LEFT', panel, 'LEFT', 11, 0)

    panel:SetScript('OnEnter', function()
        local hour, minute = GetGameTime()
        -- AU.lib.ShowSimpleTooltip(panel, string.format('Server Time: %02d:%02d', hour, minute))
    end)
...
function minimap:CreateZoomButtons(cluster)
    local zoomIn = CreateFrame('Button', 'AU_MinimapZoomIn', cluster)
    ...
    zoomIn:SetScript('OnEnter', function()
        -- AU.lib.ShowSimpleTooltip(zoomIn, 'Zoom In')
    end)
    zoomIn:SetScript('OnLeave', function()
        -- AU.lib.HideActionTooltip()
    end)

    local zoomOut = CreateFrame('Button', 'AU_MinimapZoomOut', cluster)
    ...
    zoomOut:SetScript('OnEnter', function()
        -- AU.lib.ShowSimpleTooltip(zoomOut, 'Zoom Out')
    end)
...
-- public
function AU.lib.CreateMinimapUI()
...
function AU.lib.CreateGameTimeButton(parent)
    local button = CreateFrame('Frame', 'AU_GameTimeButton', parent)
    button:SetWidth(25)
    button:SetHeight(25)
    ...
    button:SetScript('OnEnter', function()
        local hour, minute = GetGameTime()
        -- AU.lib.ShowSimpleTooltip(button, string.format('Game Time: %02d:%02d', hour, minute))
    end)
    button:SetScript('OnLeave', function()
        -- AU.lib.HideActionTooltip()
    end)

    AU.common.KillFrame(GameTimeFrame)

    return button, border, icon
end

function AU.lib.CreateCustomPlayerArrow()
    local playerArrow
...
function AU.lib.CreateButtonSkinner()
    ...
    f:SetScript('OnUpdate', function()
        if f.stop_time and GetTime() > f.stop_time then
