...
AU:NewModule('talents', 1, function()
    ...
    local function CreateMainFrame()
        frame = AU.ui.CreatePaperDollFrame('AU_TalentFrame', UIParent, 870, 600, 1)
        ...
        local headerText = AU.ui.Font(frame, 13, 'Talents', {1, 1, 1}, 'CENTER')
        ...
        frame:SetScript('OnShow', function()
            ...
            local btn = getglobal('AU_MicroButton_Talents')
        ...
        frame:Hide()
        table.insert(UISpecialFrames, 'AU_TalentFrame')
    end

    local function CreateTreeFrames()
        local xOffsets = {0, 280, 560}
        for i = 1, 3 do
            local treeFrame = CreateFrame('Frame', nil, frame)
            treeFrame:SetWidth(300)
            treeFrame:SetHeight(500)
            treeFrame:SetPoint('TOPLEFT', frame, 'TOPLEFT', xOffsets[i], -50)
    ...
    AU:NewCallbacks('talents', callbacks)
end)
