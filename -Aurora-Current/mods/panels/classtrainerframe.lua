...
AU:NewDefaults('classtrainerframe', {
    enabled = {value = true},
    version = {value = '1.0'},
})

AU:NewModule('classtrainerframe', 1, function()
    local skinned = false
    ...
    local function SkinClassTrainerFrame()
        ...
        local customBg = AU.ui.CreatePaperDollFrame('AU_ClassTrainerCustomBg', ClassTrainerFrame, 384, 512, 1)
        ...
        local closeButton = AU.ui.CreateRedButton(customBg, 'close', function() HideUIPanel(ClassTrainerFrame) end)
        closeButton:SetPoint('TOPRIGHT', customBg, 'TOPRIGHT', 0, -1)
        closeButton:SetSize(20, 20)
        closeButton:SetFrameLevel(customBg:GetFrameLevel() + 3)

        tinsert(UISpecialFrames, 'AU_ClassTrainerCustomBg')
    end
    ...
    local callbacks = {}
    AU:NewCallbacks('classtrainerframe', callbacks)
end)
